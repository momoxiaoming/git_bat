...
def down(gitPath, branch, modulesBranch):
    # workDir = os.getcwd() + "/" + workspace_dir
    # print("workDir->" + workDir)
    # if not os.path.exists(workDir):
    #     os.makedirs(workDir)
    # os.chdir(workDir)  # 先切到工作目录
    cd='git clone --depth=1 -b ' + branch + ' --recurse-submodules ' + gitPath+' ./'+workspace_dir
    print(cd)
    cmd(cd)  # 正常拉取主模块以及子模块
    os.chdir(os.getcwd() + "/" + workspace_dir)  # 再切到项目目录
    ...
    if modulesBranch is not None:
        ...
        mp=os.getcwd() + "/modules"
        cmd('rm -r ' + mp)
        if os.path.exists(mp):
            raise Exception('module clean fail')
...
def build(prjDirPath,isBuildApk):
    ...
    command = "sh gradlew "+buildType+" --stacktrace -x lint"
...
if __name__ == '__main__':
    ...
    projPath = workspacePath + "/"
    down(gitPath, gitBranch, modulesBranch)  # 下载仓库
    #复制工作目下下的local.properties到项目目录
    cp='cp ' +os.getcwd()+'/local.properties '+''+projPath+'/local.properties'
    print(cp)
    cmd(cp)
